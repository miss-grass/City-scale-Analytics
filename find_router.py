...
def generate_crossing_network(csv_file):
    '''
    Given crossing csv files, generates the network
    based on coordinates
    '''
    file = open(csv_file)
    #    G = nx.Graph()
...
def cost_art(u, v, d):
    ...
    #node_v_wt = G.nodes[v].get('node_weight', 1)
    base = 100
    art = d.get('art')  # art number
    base -= 10 * art
    # constrains
    if abs(d.get('incline')) > 0.5:
        return math.inf
    length = d.get('length')
    return length
...
def main():
    G_sdw = generate_sdwk_network("18 AU/data_table/new_sidewalks.csv")
    generate_crossing_network("18 AU/data_table/new_crossings.csv")
    #print(G_sdw.nodes)
    src_x = leftBoundary
    src_y = lowerBoundary
    dest_x = leftBoundary
